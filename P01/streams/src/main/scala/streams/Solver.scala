...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = {
    b.legalNeighbors.map(neighbor => (neighbor._1, neighbor._2 :: history)).to(LazyList)
  }
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    if (initial.isEmpty) LazyList.empty
      ...
      val initBlock = initial.head._1
      val initMoves = initial.head._2

      val initBlockNewNeighbors =
        newNeighborsOnly(neighborsWithHistory(initBlock, initMoves), explored + initBlock)

      initial ++ from(tail ++ initBlockNewNeighbors, explored + initBlock)
    }
  }
  ...
  lazy val pathsFromStart: LazyList[(Block, List[Move])] = {
    from(Set((startBlock, List.empty)).to(LazyList), Set(startBlock))
  }
  ...
  lazy val solution: List[Move] = {
    if (pathsToGoal.isEmpty) List()
      ...
      pathsToGoal.minBy(path => path._2.length)._2.reverse
    }
  }
}
